...
`From` creates an `http.Transport` using cleartext HTTP/2 and the supplied
`http.HTTP2Config`:

```go
wire := transport.From(&http.HTTP2Config{
	SendPingTimeout: 2 * time.Minute,
	PingTimeout:     20 * time.Second,
})
```

The caller constructs `http.HTTP2Config` from the configuration sources it owns.
...
```go
wire := transport.From(http2Config)
...
A nil wrapped transport selects `http.DefaultTransport`. A nil clock selects the
system clock. A nil backoff factory selects the default exponential schedule.
Callers can supply each dependency explicitly when they need custom behavior.
